const fillRatingCirles = (index, rating) => {
  const fillIndex =
...
export default function ratingCalc(rating) {
  const circles = Array.from({ length: Math.ceil(rating) }, (_, index) => {
    return fillRatingCirles(index, rating);
  });
